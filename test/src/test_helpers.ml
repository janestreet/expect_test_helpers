...
let%expect_test "[run ~hide_positions:true]" =
  ...
  in
  [%expect {|
    lib/expect_test_helpers/test/src/test_helpers.ml:LINE:COL |}]
;;
...
let%expect_test "run, with postprocess" =
  ...
  in
  [%expect {|
    lib/expect_foo_helpers/foo/src/foo_helpers.ml:LINE:COL |}]
;;
...
let%expect_test "[show_raise_async], raises hiding positions" =
  ...
  in
  [%expect {|
    (raised lib/expect_test_helpers/test/src/test_helpers.ml:LINE:COL) |}]
;;
...
let%expect_test "[require_does_not_raise_async], raises" =
  ...
  let%bind () =
    [%expect
      {|
    (* require-failed: lib/expect_test_helpers/test/src/test_helpers.ml:LINE:COL. *)
